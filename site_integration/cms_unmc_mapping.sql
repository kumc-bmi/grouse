-- cms_unmc_mapping.sql: create a mapping to integrate CMS data and blueheron data
-- Copyright (c) 2017 University of Kansas Medical Center
-- Run against the identified GROUSE data. 

-- verify that the patient_hash_map table exists for UNMC
-- Was staged in ticket:4743#comment:22
select count(*) from blueherondata_unmc.patient_hash_map;
-- 401,732
...
select * from blueherondata_unmc.patient_hash_map;

-- creating the mapping table that will have all the UNMC patients 
-- and CMS cohort along with the links between bene_ids and patient_nums
-- Note: takes a while to run (~30 minutes)
drop table cms_id.cms_unmc_mapping;
create table cms_id.cms_unmc_mapping as 
...
(
  ...
  join blueherondata_unmc.patient_hash_map mp 
