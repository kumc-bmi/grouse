-- cms_cdm_kumc_mapping_dedup.sql: explore the duplicates in the data and remove them from consideration
...
select count(*) from cms_id.cms_cdm_kumc_mapping
where bene_id is null and xw_bene_id is not null;

select count(*) ct, patient_num from (
select distinct patient_num, bene_id, bene_id_deid, 
cms_date_shift_days, BH_DATE_SHIFT_DAYS  from cms_id.cms_cdm_kumc_mapping 
where 
dups_bene_id = 0 and 
dups_pat_num = 0 and 
dups_missing_map = 0)
...
MERGE
INTO    cms_id.cms_cdm_kumc_mapping mp
USING   (
        select count(patient_num) ct, bene_id from (
        select distinct patient_num, bene_id, bene_id_deid, 
        cms_date_shift_days, BH_DATE_SHIFT_DAYS
...
select count(distinct patient_num) from cms_id.cms_cdm_kumc_mapping;

select count(distinct patid) from cdm_kumc_calamus.demographic;

select count(distinct bene_id) from cms_id.cms_cdm_kumc_mapping;
