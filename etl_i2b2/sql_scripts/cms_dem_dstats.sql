...
create or replace view demographic_summary
    ...
    , denominator as (select count(*) qty from pat)

select * from (
select 'a' row_order
    , 'Patients' statistic
    , (select qty from denominator) N
    , null "%"
    , null "Source table"
from dual
...
union all
select 'b1', 'Mean' statistic
    , (select round(avg(age_in_years_num)) from pat)
    , null
    , null
from dual
union all
select 'b2', 'Median' statistic
    , (select round(median(age_in_years_num)) from pat)
    , null
    , null
from dual

union all
select 'c', 'Age group' category, null, null, null from dual
union all
select 'c' || age_group, age_group
     , count(*) N, round(100 * count(*) / qty, 1), null
from pat cross join denominator
group by age_group, qty
...
union all
select 'e' || race_cd, race_cd
     , count(*) N, round(100 * count(*) / qty, 1), null
from pat cross join denominator
group by race_cd, qty

-- TODO: separate death stuff
union all
select 'f', 'Vital Status' category, null, null, null from dual
union all
select 'f' || vital_status_cd, vital_status_cd
     , count(*) N, round(100 * count(*) / qty, 1), null
from pat cross join denominator
group by vital_status_cd, qty
    )
order by row_order
;
